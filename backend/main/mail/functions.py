...
def send_mail(to, subject, template, **kwargs):
    ...
    try:
        ...
        return f'Mail deliver failed: {str(error)}'
    return True
...
def newsletter():
    ...
    try:
        for usuario in usuarios:
            send_mail([usuario.email], "Producto en Venta", 'newsLetter', usuario = usuario, productos = [producto.nombre for prodcucto in productos])
    except SMTPException as error:
